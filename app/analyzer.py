...
using pandas/numpy for memory-safe analysis.
"""
...
def analyze_flight_log(log_path: str) -> Dict:
    ...
    try:
        ...
        if volt_min != float("inf"):
            metrics["battery_voltage_min"] = volt_min
        else:
            metrics["battery_voltage_min"] = 0.0
        # renamed from cpu_load_max: CLpt in ms
        metrics["loop_time_ms"] = cpu_max
...
def _analyze_maneuvers(df: pd.DataFrame) -> Tuple[float, float]:
    """
    Locates rapid stick maneuvers (step inputs) and calculates exact
    ...
    for axis in ["R", "P"]:
        ...
        dt_safe = np.where(dt > 0.001, dt, 1.0)
        ...
        for step_idx in cleaned_steps:
            ...
            # Calculate overshoot
            overshoot_val = (
                (abs(max_act) - abs(target_value)) / abs(target_value)
            ) * 100
            ...
            settling_times.append(
                settled_time if settled_time is not None else 2.0
            )
