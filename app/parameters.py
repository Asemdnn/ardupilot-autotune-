...
ARDUPILOT_PARAMETERS: Dict[str, Any] = {
    "attitude_controller": {
    ...
    "vtol": {
        ...
        "parameters": {
            "VT_FW_Q_SPEED": {
                ...
                "description": (
                    "Target airspeed during VTOL-to-fixed-wing transition"
                ),
                "range": [5, 40],
                "default": 15,
                "unit": "m/s",
    ...
    "motor_outputs": {
        ...
        "parameters": {
            ...
            "MOT_SPIN_MAX": {
                ...
                "description": (
                    "Maximum motor spin as a fraction of full throttle"
                ),
                "range": [0.9, 1.0],
                "default": 0.95,
                "unit": "ratio",
        ...
        if parameter in category_data["parameters"]:
            ...
            try:
                ...
                if num_value < param_info["range"][0]:
                    msg = f"Value {value} is below minimum {param_info['range'][0]}"  # noqa
                    return False, msg
                if num_value > param_info["range"][1]:
                    msg = f"Value {value} exceeds maximum {param_info['range'][1]}"  # noqa
                    return False, msg

                msg = (
                    f"Value is within safe range "
                    f"[{param_info['range'][0]}, {param_info['range'][1]}]"
                )
                return True, msg
...
def format_recommendations(recommendations: list) -> str:
    ...
    for i, rec in enumerate(recommendations, 1):
        ...
        curr = rec.get("current", "N/A")
        recm = rec.get("recommended", "N/A")
        output += f"   Current: {curr} → Recommended: {recm}\n"
